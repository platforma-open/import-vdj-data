...
- The requested feature is honoured whenever the header carries its regions, even if the file never fills them. MiXCR's `exportAirr` emits an `fwr1` column it leaves empty on an FR1-primer amplicon; asking for VDJRegion there now discards every row and says so in the counters, rather than quietly importing CDR3 clonotypes nobody asked for. Only a region the file has no column for at all falls back.
- Whether rows carry a cell barcode follows the format the scientist declared, not whether the file has a `cell_id` column. A bulk AIRR export carrying an empty `cell_id` imports as bulk.
